...
class Environment(BaseEnvironment):
        ...
        def check_cell(pos):
            for i, geese in enumerate(obs['geese']):
                if pos in geese:
                    ...
                    pos_next = geese[index +
                                     1] if index < len(geese) - 1 else None
                    directions = [self.direction(
                        pos, pos_prev), self.direction(pos, pos_next)]
                    return i, directions
            if pos in obs['food']:
                return 'f'
            return None
    ...
    def outcome(self):
        ...
        rewards = {o['observation']['index']: o['reward']
                   for o in self.obs_list[-1]}
    ...
    def rule_based_action(self, player):
        ...
        action_map = {'N': Action.NORTH, 'S': Action.SOUTH,
                      'W': Action.WEST, 'E': Action.EAST}
